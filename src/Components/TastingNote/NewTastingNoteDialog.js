...
import { createTastingNote } from '../../Redux/actions/tastingNotes'
import { fetchScotches } from '../../Redux/actions/scotches'

import '../../App.css';

class NewTastingNoteDialog extends Component {
  ...
  async save() {
    this.props.createTastingNote(this.state.tastingNote)
    this.setState({
      tastingNote: {},
      showDialog: false
    })
        ...
        <Dialog visible={this.state.showDialog} style={{width:'25vw'}} header={'New Tasting Note'} modal={true} footer={dialogFooter} onHide={this.cancel}>
          {
            ...
            <div className="p-col-8">
              <Dropdown id="scotchId" value={this.state.tastingNote.scotchId} options={this.props.scotches} optionLabel="dramName" optionValue="id" style={{width: '15em'}} scrollHeight='200px' onChange={(e) => {this.updateProperty('scotchId', e.target.value)}} />
            </div>
...
const mapDispatchToProps = { fetchScotches, createTastingNote }

const mapStateToProps = state => ({
  scotches: state.scotches.scotches
})

export default connect(mapStateToProps, mapDispatchToProps)(NewTastingNoteDialog)